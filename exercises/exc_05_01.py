...
def resnet_block(X, F, k, downsample):
    ...
    # shortcut
    if downsample == True:
        x_shortcut = layers.Conv2D(F, (_____,_____), padding="_____", strides=(____,_____)) (X) # decrease input size as well
    else:
    ...
    if downsample == True:
        ...
        x = layers.Conv2D(F, (k,k), padding="same", strides=(____,____)) (X) # normal path
...
def model():
    ...
    conv=layers.Conv2D(64, (____,_____), padding="same", strides=(____,_____)) (input_)
    conv=layers.Activation("relu") (conv)
    conv = layers.__________ (conv)
    
    block_1 = resnet_block(conv, ____, _____, downsample=False)
    block_1 = resnet_block(block_1, 64, 3, downsample=False)
    block_1 = resnet_block(block_1, 64, 3, downsample=False)
    
    block_2 = resnet_block(block_1, _____, 3, downsample=_____)
