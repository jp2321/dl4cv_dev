...
def model(trainable=True, number_of_layers=None):
    ...
    if number_of_layers != None: # Check if number of layers is set 
        for i in range(0, _________): # Set only for those layers the trainable parameter
            transfer_learning_model.layers[i].trainable=_________
    else: # otherwise set it for the transfer_learning_model part
